...
var Agent = function () {
  function Agent() {
    ...
    var autoloads = ['browser', 'platform', 'mobile', 'tablet', 'desktop', 'touch', 'viewport'];
    ...
    if (autoload) {
      ...
      try {
        for (var _iterator = Object.keys(notchedScreens)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var key = _step.value;

          if (screen.pixelWidth == notchedScreens[key][0] && screen.pixelHeight == notchedScreens[key][1] || screen.pixelWidth == notchedScreens[key][1] && screen.pixelHeight == notchedScreens[key][0]) {
  ...
  _createClass(Agent, [{
    ...
    get: function get() {
      if (this.attributes['data-browser']) {
        var values = this.attributes.getNamedItem('data-browser').value.split(' ');
        return {
          name: values[0] || null,
